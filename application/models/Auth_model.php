...
class Auth_model extends CI_Model
{
    ...
    public function get_paginated_filtered_users($role, $per_page, $offset)
    {
        ...
        $this->db->join('location_countries','users.country_id =location_countries.id','left');
    ...
    public function filter_users($role)
    {
        ...
        $this->db->select('users.*, roles_permissions.role_name AS role_name_array, roles_permissions.is_super_admin AS is_super_admin, location_countries.name as country');
        $this->db->join('roles_permissions', 'roles_permissions.id = users.role_id');
       /* $countries =input_get('countries');
        if(!empty($countries)){
            $this->db->where('location_countries.name',$countries);
        }else{
